...
- 机の上に貼る**面ごとに色を持つメッシュ**として出力

色が依存する姿勢は `--color-by` で選ぶ。既定は `top_down`。

| 色 | 意味（既定の `top_down`） |
|---|---|
| 緑 | 上から 4 yaw すべて解ける（どの向きで置いても掴める） |
| 黄 | 上からのどれかが解ける（向きによっては掴める） |
| 赤 | 上からは掴めない |

横方向の結果は JSON に `summary_by_family` として残るが、色には使わない。
8 姿勢を混ぜて色を決めると「上から 4 通り全部いける良い場所」と「変な横方向 1 つだけ
いける場所」が同じ黄色になり、配置の判断材料にならないため。
`--color-by all` で従来の混合、`--color-by side` で横方向のみに切り替えられる。
IK 結果は `ik_success.npy` に残るので、色を変えても**再計算は起きない**。
...
proxy tool の重心は**掴む部位（頭）の中心に明示的に置いてある**。

これはシーン側の設定であって認識の話ではない。寸法・質量・摩擦と同じく、こちらが決める
物体の性質で、プランナも実行も重心を読まない（読むのは PhysX だけ）。ハンマーもナイフも
ここで掴む部位に質量が集中しているので、実物に寄せた設定でもある。

**ただし、これで難易度をひとつ下げていることは自覚しておく。** 均一密度のままだと重心は
把持点から 47.5 mm ずれ、重力トルクで道具が指の中で 74° お辞儀して、柄が真下を向いた
まま handover に到達した（実測）。本来これは握力で解くべき問題で、今回は回避した。

Phase 2 で実メッシュに替えると重心はアセット側の性質になり、**こちらで選べなくなる**。
そこで再びお辞儀するなら握力で解く番で、そのために `execute_handover.py` は指のドライブ
諸元（stiffness / damping / max_force）をステージから読んでログに出す。実機 Franka の
把持力は約 70 N なので、アセットがそれ未満なら上げる余地がある。
